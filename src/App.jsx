...
function App() {
  ...
  const [VoiceAccentOutput, setVoiceAccentOutput] = useState('UK English Female');
  ...
  const handleButtonClick = () => {
    ...
    if (CountInput < 500) {
      ...
      fetch(`https://api.mymemory.translated.net/get?q=${inputValue}&langpair=${LangInput}|${LangOutput}`)
    ...
    <main className='text-text font-bold font flex flex-col items-center sm:justify-center'>
      ...
      <section className='flex-col lg:flex-row ml-auto mr-auto lg:max-w-4xl flex pb-60 sm:gap-4 mt-10 justify-center'>
        <article className='flex flex-col items-center border-[0.25px] sm:p-6 sm:mt-6 m-1 p-3 rounded-3xl bg-firstarticle sm:min-w-[40vw] border-nonselected'>
          <div className='flex flex-col w-fit sm:w-full sm:justify-start sm:flex-row gap-4 text-center justify-center mt-2 sm:ml-4 relative after:content{} sm:items-center 
        ...
        if (index < 2) {
          return (
            <button
              ...
              className={`px-2 sm:px-4 py-1 sm:py-2 ${selectedButtonInput === button.code ? 'bg-nonselected rounded-xl font-medium sm:px-3 sm:py-2 px-1.5 py-1 text-white' : 'text-nonselected'}`}
              onClick={() => HandleLangInput(button)}
            >
              {button.name}
            </button>
          );
        } else if (index === 2) { // Quand l'index est 2, commencez un `select`
          return (
            <Select 
              key="select-lang"
              id="languages" 
              required
              className={`px-4 py-2 ${
                selectedButtonInput === 'es' || selectedButtonInput === 'de' ? 'bg-nonselected rounded-xl font-medium px-3 py-2 text-white' : 'bg-transparent border-none text-nonselected'
              }`}
              class='bg-transparent'
              value={selectedButtonInput}
              onChange={(e) => {
                const newLangCode = e.target.value;
                const button = buttons_input.find(b => b.code === newLangCode);
          ...
          <div className='flex gap-2 justify-between sm:gap-0 mt-4 items-center w-full'>
            ...
            <div className='flex justify-center items-center p-4 py-3 px-7 border-nonselected border-1 rounded-xl bg-bgbutton' onClick={handleButtonClick}>
              <img 
              src={A_Translate} 
              alt="" 
              className='mr-2 h-[30.5px]'
              />
        ...
        if (index < 2) {
          return (
            <button
              key={button.code} // Il est préférable d'utiliser button.code si c'est unique
              className={`px-3 py-2 ${selectedButtonOutput === button.code ? 'bg-nonselected rounded-xl font-medium px-3 py-2 text-white' : 'text-nonselected'}`}
          ...
          return (
            <Select 
              ...
              required
              className={`px-3 py-2 ${
                selectedButtonOutput === 'es' || selectedButtonOutput === 'de' ? 'bg-nonselected rounded-xl font-medium px-3 py-2 text-white' : 'bg-transparent border-none text-nonselected'
              }`}
              class='bg-transparent'
              value={selectedButtonOutput}
              onChange={(e) => {
                const newLangCodeOutput = e.target.value;
                const button = buttons_output.find(b => b.code === newLangCodeOutput);
            ...
            <div className='flex justify-center sm:justify-start mr-6 sm:mr-0'>
              ...
              src={change} 
              alt="" 
              onClick={() => SwapLangage(selectedButtonInput, selectedButtonOutput, VoiceAccentInput, VoiceAccentOutput, setSelectedButtonInput, setSelectedButtonOutput, setVoiceAccentInput, setVoiceAccentOutput, setLangInput, setLangOutput, buttons_input, buttons_output)}
